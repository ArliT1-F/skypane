...
export const VEHICLES = {
  coupe: {
    ...
    // side profile [x (forward), y] from rear bottom, counter-clockwise
...
function buildBody(v, color) {
  ...
  // extrude goes along +Z, profile x = forward. Rotate so forward = +Z and centre across X.
  ...
  if (v.twoTone) {
    // cream upper half for the camper
  ...
  // side windows: inset glass prisms slightly wider than the body
  ...
  if (!v.twoTone) {
    // windscreen & rear window as thin slanted panels (approximate)
    ...
    group.add(ws);
    // rear window along the back slope of the glasshouse
...
export class Car {
  constructor(scene) {
    this.scene = scene;
    this.root = new THREE.Group(); // yaw + position
    this.chassis = new THREE.Group(); // pitch/roll + suspension
    this.root.add(this.chassis);
    scene.add(this.root);

    // headlights (spot lights, no shadows for performance)
    this.headlight = new THREE.SpotLight(0xfff1d6, 0, 90, 0.55, 0.55, 1.2);
  ...
  setNight(n) {
    this.headMat.emissiveIntensity = 0.2 + n * 3;
    this.headlight.intensity = n * 60;
    this.tailBase = 0.25 + n * 1.2;
  }
  ...
  placeOnRoad(world, index) {
    ...
    const off = -LANE_WIDTH / 2; // right lane
  ...
  update(dt, world, input, autodrive) {
    ...
    if (autodrive) {
      // pure-pursuit toward a point ahead in the right lane
      const look = clamp(8 + this.speed * 0.9, 10, 42);
      ...
      // choose a speed based on upcoming curvature
      let maxK = 0;
      for (let k = 0; k < 90; k += 3) maxK = Math.max(maxK, Math.abs(world.curvatureAt(Math.min(world.length - 1, i + k))));
      const target = Math.min(this.cruise ?? v.maxSpeed * 0.62, Math.sqrt(3.2 / Math.max(maxK, 1e-4)));
    ...
    const steerTarget = steer * v.maxSteer * limit;
    const rate = autodrive ? 3 : Math.abs(steerTarget) > Math.abs(this.steer) ? 2.4 : 4.5;
    this.steer += clamp(steerTarget - this.steer, -rate * dt, rate * dt);

    // ------------------------------------------------ longitudinal
    const g = 9.81;
    const slope = this.pitchSlope || 0; // rise over run along heading
    ...
    if (brake > 0) {
      ...
      else acc -= brake * 3; // reverse gently
    }
    acc -= g * slope / Math.sqrt(1 + slope * slope);
    acc -= this.speed * 0.012 + this.speed * Math.abs(this.speed) * 0.00035; // rolling + air
    ...
    // ------------------------------------------------ yaw & move
    const yawRate = (this.speed * Math.tan(this.steer)) / v.wheelBase;
    if (!this.airborne) this.heading += yawRate * dt;
    const nfx = Math.sin(this.heading), nfz = Math.cos(this.heading);
    this.x += nfx * this.speed * dt;
    this.z += nfz * this.speed * dt;
    this.distance += Math.abs(this.speed) * dt;
    void fx; void fz;

    // ------------------------------------------------ ground contact (4 wheels)
    const hw = v.track / 2, hb = v.wheelBase / 2;
    const rx = Math.cos(this.heading), rz = -Math.sin(this.heading); // left vector
    const hFL = this.groundAt(world, this.x + nfx * hb + rx * hw, this.z + nfz * hb + rz * hw);
    const hFR = this.groundAt(world, this.x + nfx * hb - rx * hw, this.z + nfz * hb - rz * hw);
    const hRL = this.groundAt(world, this.x - nfx * hb + rx * hw, this.z - nfz * hb + rz * hw);
    const hRR = this.groundAt(world, this.x - nfx * hb - rx * hw, this.z - nfz * hb - rz * hw);
    const ground = (hFL + hFR + hRL + hRR) / 4;
    const pitchT = Math.atan2((hFL + hFR) / 2 - (hRL + hRR) / 2, v.wheelBase);
    const rollT = Math.atan2((hFL + hRL) / 2 - (hFR + hRR) / 2, v.track);
    ...
    // cosmetic body motion: lean in corners, squat under throttle, dive under braking
    const lat = yawRate * this.speed; // lateral acceleration
    this.bodyRoll = lerp(this.bodyRoll || 0, clamp(-lat * 0.006, -0.06, 0.06), 1 - Math.exp(-dt * 5));
    ...
    this.body.position.y = 0.02 - this.landBump + rumble;
    ...
    // fell into a lake or far off the map → back to the road
    return this.y < WATER_LEVEL - 1.2;
  }

  steerLimit() {
    // less steering lock at speed keeps things calm and stable
    return 1 / (1 + Math.abs(this.speed) * 0.045);
  }
}
