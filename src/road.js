...
function makeRoadTexture(maxAniso) {
  ...
  // Soft textured gravel shoulders
  g.fillStyle = '#6b665c';
  g.fillRect(0, 0, W, H);
  ...
  // Subtle tyre-worn wheel tracks
  g.fillStyle = 'rgba(18,18,22,0.16)';
  for (const off of [-2.6, -1.0, 1.0, 2.6]) {
    const x = px(SHOULDER + ROAD_HALF + off);
    g.fillRect(x - px(0.35), 0, px(0.7), H);
  }

  // Crisp edge lines (solid white)
  g.fillStyle = '#eae7dd';
  const lw = px(0.16);
  g.fillRect(a0 + px(0.22), 0, lw, H);
  g.fillRect(a1 - px(0.22) - lw, 0, lw, H);

  // Centre line: dashed warm golden amber (4 m on / 8 m off over a 12 m tile)
  g.fillStyle = '#f4be5e';
  const cx = px(SHOULDER + ROAD_HALF);
  g.fillRect(cx - lw / 2, 0, lw, (4 / TEX_REPEAT) * H);

  // Retroreflective center road studs (cat's eyes)
  g.fillStyle = '#ffffff';
  g.fillRect(cx - px(0.06), (1.5 / TEX_REPEAT) * H, px(0.12), px(0.12));
...
export class Road {
  constructor(scene, world, renderer) {
    ...
    this.postMat = new THREE.MeshStandardMaterial({ color: 0xf3f1ea, roughness: 0.55 });
    const refGeo = new THREE.BoxGeometry(0.13, 0.14, 0.13);
    refGeo.translate(0, 0.85, 0);
    this.refGeo = refGeo;
    this.refMat = new THREE.MeshStandardMaterial({ color: 0xff9533, emissive: 0xff8520, emissiveIntensity: 0.2, roughness: 0.25 });
  }

  setNight(n) {
    this.refMat.emissiveIntensity = 0.2 + n * 2.5;
  }
  ...
  _build(k) {
    ...
    // Reflector posts along both shoulders
    const posts = [];
